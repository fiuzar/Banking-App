...
import { 
  ...
  Download, RefreshCcw, ShoppingBag, 
...
// Mock Data following your specific FX pairing rule
const transactionGroups = [
  {
    date: "Today, Jan 12, 2026",
    items: [
      { id: 1, title: "Amazon EU", sub: "Card Purchase", amount: "-€120.00", currency: "EUR", type: "debit", icon: <ShoppingBag size={18} /> },
    ]
  },
  {
    date: "Jan 11, 2026",
    items: [
      { id: 2, title: "FX Conversion", sub: "To EUR Wallet", amount: "-$1,000.00", currency: "USD", type: "fx", icon: <RefreshCcw size={18} /> },
      { id: 3, title: "FX Conversion", sub: "From USD Wallet", amount: "+€917.00", currency: "EUR", type: "fx", icon: <RefreshCcw size={18} /> },
    ]
  },
  {
    date: "Jan 10, 2026",
    items: [
      { id: 4, title: "Salary Credit", sub: "TechCorp Intl", amount: "+$3,200.00", currency: "USD", type: "credit", icon: <ArrowDownLeft size={18} /> },
    ]
  }
];
...
export default function HistoryPage() {
  return (
    <div className="max-w-3xl mx-auto pb-20">
      {/* 1. Header & Search */}
      <div className="sticky top-16 z-30 bg-n-100/80 backdrop-blur-md pt-4 pb-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" className="rounded-full">
              <ArrowLeft className="w-5 h-5" />
            </Button>
            <h1 className="text-2xl font-bold text-brand-dark">History</h1>
          </div>
          
          <div className="flex gap-2">
            <Button variant="outline" size="icon" className="rounded-full border-n-300">
              <Download className="w-4 h-4 text-n-500" />
            </Button>
        ...
        <div className="relative">
          ...
          <Input 
            ...
            className="h-12 pl-12 bg-white border-n-300 rounded-brand-button focus:ring-brand-blue/10" 
          />
        </div>
      </div>
      ...
      <div className="space-y-8">
        {transactionGroups.map((group) => (
          <div key={group.date} className="space-y-3">
            <h3 className="text-[11px] font-bold text-n-500 uppercase tracking-widest px-2">
              {group.date}
            </h3>
            <div className="bg-white rounded-brand-card shadow-soft overflow-hidden border border-n-100">
...
function TransactionRow({ tx, isLast }) {
  ...
  return (
    <div className={`flex items-center justify-between p-4 hover:bg-n-100/50 transition-colors cursor-pointer ${!isLast ? 'border-b border-n-100' : ''}`}>
      <div className="flex items-center gap-4">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center 
          ${tx.type === 'fx' ? 'bg-brand-blue/10 text-brand-blue' : 
            isCredit ? 'bg-usd/10 text-usd' : 'bg-n-100 text-n-500'}`}>
          {tx.icon}
        </div>
        <div>
      ...
      <div className="flex flex-col items-end gap-1">
        <span className={`text-sm font-mono font-bold ${isCredit ? 'text-usd' : 'text-brand-dark'}`}>
          {tx.amount}
        </span>
        <Badge variant="outline" className="text-[9px] font-bold px-1.5 py-0 h-4 border-n-300 text-n-500">
          {tx.currency}
        </Badge>
      </div>
    </div>
...
function FilterSheet() {
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="rounded-full border-n-300">
          <SlidersHorizontal className="w-4 h-4 text-n-500" />
        </Button>
      </SheetTrigger>
      <SheetContent side="bottom" className="rounded-t-[2rem] p-8 max-h-[85vh]">
        <SheetHeader>
          <SheetTitle className="text-xl font-bold text-left">Filter Transactions</SheetTitle>
        </SheetHeader>
        
        <div className="py-8 space-y-8">
          {/* Currency Filter */}
          <div className="space-y-3">
            <p className="text-xs font-bold text-n-500 uppercase tracking-wider">Currency</p>
            <div className="flex gap-3">
              {['All', 'USD', 'EUR'].map(c => (
                <button key={c} className={`px-6 py-2 rounded-full border text-sm font-bold transition-all ${c === 'All' ? 'bg-brand-blue border-brand-blue text-white' : 'border-n-300 text-n-500'}`}>
                  {c}
                </button>
              ))}
            </div>
          ...
          {/* Type Filter */}
          <div className="space-y-3">
            <p className="text-xs font-bold text-n-500 uppercase tracking-wider">Transaction Type</p>
            <div className="flex flex-wrap gap-2">
              {['Credits', 'Debits', 'FX Conversions', 'Loans'].map(t => (
                <button key={t} className="px-4 py-2 rounded-lg border border-n-100 bg-n-100/50 text-xs font-bold text-n-700">
                  {t}
                </button>
              ))}
            </div>
        ...
        <div className="flex gap-4 pt-4">
          <Button variant="ghost" className="flex-1 font-bold text-n-500">Reset</Button>
          <Button className="flex-1 btn-primary h-12">Apply Filters</Button>
        </div>
      </SheetContent>
    </Sheet>
  );
