...
import { useState, useEffect, useContext, use } from "react" // Note the 'use' hook for params
import { Button } from "@/components/ui/button"
import { AccountDetailsContext } from "@/server-functions/contexts"
import {
	ArrowLeft, ArrowUpRight, ArrowDownLeft,
	Search, Filter, Download, CreditCard, UserPlus
} from "lucide-react"
...
export default function AccountDetails() {
    ...
    // Unwrap the dynamic route parameter (savings or checking)
	...
	useEffect(() => {
        // Reset and load when account type changes
    ...
    async function loadTransactions(currentOffset, isInitial = false) {
        if (!user_id) return
        setLoading(true)
        const { success, history, hasMore: moreAvailable } = await get_transaction_history_list(account_type, currentOffset)
        
        if (success) {
            setTransactionHistory(prev => isInitial ? history : [...prev, ...history])
		...
		<div className="min-h-screen bg-slate-50">
			...
			<div className="bg-primary pt-8 pb-32 px-6 text-white relative">
				<div className="flex justify-between items-center mb-8">
					...
					<h1 className="text-sm font-bold uppercase tracking-widest opacity-80">Account Details</h1>
				...
				<div className="space-y-1 text-center">
					...
					<h1 className="text-5xl font-black tracking-tighter">
						${isSavings 
                            ? parseFloat(accountDetails?.savings_balance || 0).toLocaleString(undefined, {minimumFractionDigits: 2}) 
        ...
        <CreditCard size={14} className="text-white/40" />
			...
			<div className="max-w-md mx-auto px-6 -mt-12 relative z-20">
				<div className="bg-white rounded-3xl p-2 shadow-xl border border-slate-100 flex gap-2">
					<Link href={`/app/deposit/card?account=${account_type}`} className="flex-1">
						<Button className="w-full h-14 bg-green-800 hover:bg-green-900 rounded-2xl flex flex-col items-center justify-center">
							<CreditCard size={18} />
							<span className="text-[10px] font-bold uppercase mt-1">Deposit</span>
						</Button>
					</Link>
					<Link href={`/app/deposit/request?account=${account_type}`} className="flex-1">
						<Button variant="outline" className="w-full h-14 border-slate-200 text-slate-700 rounded-2xl flex flex-col items-center justify-center">
							<UserPlus size={18} />
			...
			<div className="bg-white mt-8 min-h-[60vh] p-6 rounded-t-[40px] shadow-2xl relative z-10">
				<div className="flex justify-between items-center mb-6 pt-2">
					<h2 className="text-lg font-black text-brand-dark">Transaction History</h2>
					<div className="flex gap-2">
						<button className="p-2 bg-slate-50 rounded-full text-slate-400"><Search size={18} /></button>
						<button className="p-2 bg-slate-50 rounded-full text-slate-400"><Filter size={18} /></button>
					</div>
				</div>
