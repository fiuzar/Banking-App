...
export default function TicketDetailPage() {
    ...
    // 1. Fetch function for initial load and polling
    ...
    // 2. Initial Load & Polling for new Admin replies
    useEffect(() => {
        fetchData(true);
    ...
    const handleSendReply = async () => {
        ...
        try {
            // Use the conversation_id found by getTicketDetails
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-32">
            ...
            <div className="bg-white border-b px-6 py-4 flex items-center gap-4 sticky top-0 z-10 shadow-sm">
                ...
                <div>
                    <h1 className="text-sm font-black text-slate-900 leading-tight">Ticket #{ticket.id.slice(-5).toUpperCase()}</h1>
                    <div className="flex items-center gap-2 mt-0.5">
                        <div className={`w-1.5 h-1.5 rounded-full ${ticket.status === 'OPEN' ? 'bg-blue-500' : 'bg-green-500'}`} />
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{ticket.status}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-md mx-auto p-6 space-y-8">
                            ...
                            <div className={`p-4 rounded-[24px] shadow-sm border border-slate-100 ${
                                ...
                                <div className="flex justify-between items-center mb-1.5">
                                    <p className="text-[10px] font-black text-slate-900 uppercase tracking-wide">
                                        {reply.sender_role === 'ADMIN' ? 'Support Agent' : 'You'}
                                    </p>
                ...
                {ticket.status === 'RESOLVED' && (
                    <div className="bg-green-500 text-white p-5 rounded-[24px] flex items-center gap-4 shadow-lg shadow-green-100 animate-in fade-in zoom-in duration-300">
                        ...
                        <div>
                            <p className="text-xs font-black uppercase tracking-widest">Issue Resolved</p>
