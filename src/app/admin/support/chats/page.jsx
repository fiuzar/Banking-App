...
export default function AdminChatPage() {
    ...
    return (
        <div className="h-screen bg-slate-50 flex flex-col lg:flex-row">
            ...
            <div className="flex-1 flex flex-col h-full bg-white">
                ...
                <div className="p-4 border-b flex items-center justify-between bg-white sticky top-0 z-10">
                    <div className="flex items-center gap-4">
                        <Link href="/admin/support" className="p-2 hover:bg-slate-100 rounded-full">
                            <ArrowLeft size={20} />
                        </Link>
                        <div>
                            <h2 className="font-black italic text-slate-900 leading-none">{ticket?.subject}</h2>
                            <p className="text-[10px] font-bold text-primary uppercase mt-1">ID: {ticket?.id}</p>
                        </div>
                    </div>
                    <Button 
                    ...
                    {messages.map((msg, i) => (
                        <div key={i} className={`flex ${msg.sender_role === 'ADMIN' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[70%] p-4 rounded-2xl shadow-sm ${
                                ...
                                <p className={`text-[9px] mt-2 font-bold uppercase opacity-50 ${msg.sender_role === 'ADMIN' ? 'text-right' : 'text-left'}`}>
                ...
                <div className="p-4 bg-white border-t">
                    <div className="max-w-4xl mx-auto relative flex items-end gap-3">
                        <Textarea 
                            ...
                            className="flex-1 min-h-[100px] rounded-2xl bg-slate-50 border-none focus-visible:ring-primary/20 resize-none p-4 text-sm font-medium"
                        />
                        <Button 
                            disabled={sending || !newMessage.trim()}
                            onClick={() => handleSend(false)}
                            className="h-14 w-14 rounded-2xl bg-primary shadow-lg shadow-primary/30"
                        >
            ...
            {/* User Sidebar Info */}
            <aside className="w-full lg:w-80 bg-slate-50 border-l p-6 space-y-6 overflow-y-auto">
                <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm text-center">
                    ...
                    <h3 className="font-black text-slate-900 text-lg leading-tight">{ticket?.first_name} {ticket?.last_name}</h3>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{ticket?.email}</p>
                </div>

                <div className="space-y-4">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase ml-2">Ticket Metadata</h4>
                    <div className="bg-white rounded-2xl p-4 border border-slate-200 space-y-3">
                        <InfoRow icon={<Clock size={14}/>} label="Created" value={new Date(ticket?.ticket_created).toLocaleDateString()} />
                        <InfoRow icon={<Mail size={14}/>} label="Category" value={ticket?.type} />
                        <div className="pt-2 border-t mt-2">
                            <p className="text-[9px] font-black text-slate-300 uppercase mb-1">Initial Description</p>
                            <p className="text-xs text-slate-600 font-medium leading-relaxed italic">"{ticket?.description || 'No description provided'}"</p>
                        </div>
                    </div>
                </div>
            </aside>
