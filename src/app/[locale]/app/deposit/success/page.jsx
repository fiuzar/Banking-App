        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center animate-in zoom-in-95 duration-500">
            ...
            <div className="bg-white border border-slate-100 p-6 rounded-[32px] shadow-sm mb-8 w-full max-w-xs">
                <p className="text-slate-400 text-xs uppercase font-bold tracking-widest mb-1">
                    {t("DepositStatus", "pending.amountLabel")}
                </p>
                ...
                <p className="text-[10px] text-slate-400 font-medium leading-relaxed">
                    {t("DepositStatus", "pending.verificationNote", { account: account || "" })}
                </p>
            </div>
            ...
            <div className="w-full max-w-xs space-y-4">
                <Link href="/app">
                    <Button className="w-full h-16 bg-green-900 hover:bg-green-800 text-white font-black rounded-3xl shadow-lg shadow-green-900/20 flex items-center justify-center gap-2 transition-all active:scale-95">
                        {t("DepositStatus", "actions.dashboard")} <ArrowRight size={18} />
                    </Button>
                </Link>
                
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                    <ShieldCheck size={12} /> {t("DepositStatus", "pending.secureTag")}
                </p>
            </div>
        </div>
    )
    ...
    return (
        <Suspense fallback={
            <div className="min-h-screen flex items-center justify-center">
                ...
                <p className="text-xs text-slate-400 mt-2">{t("DepositStatus", "loading")}</p>
            </div>
        }>
            <SuccessContent />
        </Suspense>
