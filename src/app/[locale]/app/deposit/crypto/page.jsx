...
export default function CryptoDeposit() {
    ...
    const handleSelectCoin = async (coin) => {
        ...
        if (res.result?.address) setAddress(res.result.address)
        setLoading(false)
    }
        ...
        <div className="min-h-screen bg-slate-50 pb-10">
            ...
            <div className="bg-green-900 pt-8 pb-24 px-6 text-white relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 relative z-10">
                    ...
                    <h1 className="text-xl font-bold">{t("CryptoDeposit", "header.title")}</h1>
                </div>
            </div>
            ...
            <div className="px-6 -mt-12 max-w-md mx-auto relative z-20">
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="bg-white p-4 rounded-[32px] shadow-sm border border-slate-100">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-2">
                                {t("CryptoDeposit", "selection.label")}
                             </p>
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="text-left">
                                                ...
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">
                                                    {t("CryptoDeposit", "selection.networkPrefix")} <span className="text-green-700">{coin.network}</span>
                                                </p>
                                            </div>
                                        </div>
                    ...
                    <div className="bg-white p-8 rounded-[40px] shadow-2xl border border-slate-100 text-center space-y-6 animate-in zoom-in-95 duration-300">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                                {t("CryptoDeposit", "wallet.secureTitle", { symbol: selectedCoin.symbol })}
                            </span>
                        ...
                        {loading ? (
                            <div className="py-16 flex flex-col items-center gap-4">
                                ...
                                <p className="text-xs font-bold text-slate-400 animate-pulse">{t("CryptoDeposit", "wallet.loading")}</p>
                            </div>
                        ) : (
                            <>
                                ...
                                <div className="space-y-3">
                                    ...
                                    >
                                        ...
                                        <div className="mt-2 flex items-center gap-1.5 text-green-700">
                                            {copied ? (
                                                <><Check size={14} className="font-bold" /> <span className="text-[10px] font-black uppercase">{t("CryptoDeposit", "wallet.copied")}</span></>
                                            ) : (
                                                <><Copy size={14} /> <span className="text-[10px] font-black uppercase">{t("CryptoDeposit", "wallet.copy")}</span></>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-2 px-4 py-3 bg-red-50 rounded-2xl border border-red-100">
                                        ...
                                        <p className="text-[9px] text-red-600 font-bold text-left leading-tight">
                                            {t("CryptoDeposit", "wallet.warning", { network: selectedCoin.network })}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="pt-2">
                                    ...
                                    >
                                        {t("CryptoDeposit", "wallet.changeAsset")}
                                    </button>
                                </div>
                            </>
                        )}
                ...
                <div className="mt-8 flex flex-col items-center gap-4">
                    <div className="flex items-center gap-2 text-slate-400">
                        <ShieldCheck size={14} />
                        <span className="text-[10px] font-bold uppercase tracking-widest">{t("CryptoDeposit", "security.encryption")}</span>
                    </div>
                    <p className="text-[9px] text-slate-400 text-center px-8 leading-relaxed">
                        {t("CryptoDeposit", "security.footerNote")}
                    </p>
                </div>
            </div>
        </div>
