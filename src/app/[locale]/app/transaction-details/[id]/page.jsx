...
import ReceiptJSON from "@/locales/Receipt.json"
...
export default function TransactionDetails() {
  ...
  if (!tx) return (
    <div className="p-20 text-center">
      ...
      <Link href="/app">{ReceiptJSON.actions.support}</Link>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 print:bg-white print:p-0">
      ...
      <div className="bg-white p-6 flex justify-between items-center border-b border-slate-100 print:hidden">
        ...
        <h1 className="text-[10px] font-black uppercase tracking-widest text-slate-400">{ReceiptJSON.header.title}</h1>
      ...
      <div className="max-w-md mx-auto p-6 space-y-6 print:p-0 print:m-0 print:max-w-full">
        ...
        <div className="bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 text-center relative overflow-hidden print:border-none print:shadow-none">
          ...
          <div className="hidden print:flex flex-col items-center mb-10">
            ...
            <h1 className="text-xl font-black text-slate-900 tracking-tighter">{ReceiptJSON.ledger.title}</h1>
            <p className="text-[8px] uppercase tracking-[0.3em] text-slate-400 font-bold">{ReceiptJSON.ledger.subtitle}</p>
          </div>
          ...
          <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${tx.status === 'completed' ? 'text-green-600 bg-green-50' : 'text-amber-600 bg-amber-50'} print:border print:border-green-100`}>
            {ReceiptJSON.status[tx.status] || tx.status}
          </div>
          ...
          <div className="space-y-6 text-left relative z-10">
            <DetailRow label={ReceiptJSON.details.dateTime} value={format(new Date(tx.created_at), "MMM dd, yyyy • hh:mm b")} />
            <DetailRow label={ReceiptJSON.details.referenceId} value={tx.reference_id} isMono />
            <DetailRow label={ReceiptJSON.details.accountType} value={ReceiptJSON.details.accountMask} />
            <DetailRow label={ReceiptJSON.details.transactionStatus} value={ReceiptJSON.status.settled} />
          </div>
          ...
          <div className="hidden print:block mt-16 pt-8 border-t border-slate-100 text-[9px] text-slate-400 text-center leading-relaxed">
            <p className="font-bold">{ReceiptJSON.footer.electronicReceipt}</p>
            <p>{ReceiptJSON.footer.generatedOn} {format(new Date(), "PPpp")}</p>
            <p className="mt-2">{ReceiptJSON.footer.hash}: {tx.reference_id}-{tx.id}-secure</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 px-5 py-4 bg-white rounded-[24px] border border-slate-100 shadow-sm print:hidden">
          ...
          <p className="text-[10px] text-slate-500 font-bold leading-snug uppercase tracking-tight">
            {ReceiptJSON.security.shield} <br/>
            <span className="text-slate-300">{ReceiptJSON.security.authCode}: PS-990{tx.id}</span>
          </p>
        </div>

        {/* ACTION BUTTONS */}
        <div className="grid grid-cols-2 gap-4 print:hidden">
          <Button onClick={handlePrint} className="h-14 bg-green-800 text-white font-black rounded-2xl flex gap-2 shadow-lg shadow-green-900/20">
            <Printer size={18} /> {ReceiptJSON.actions.print}
          </Button>
          <Button variant="outline" className="h-14 border-slate-200 text-slate-400 font-black rounded-2xl">
            {ReceiptJSON.actions.support}
          </Button>
        </div>
      </div>
    </div>
